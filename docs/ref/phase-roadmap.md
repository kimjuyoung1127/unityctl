...
| 우선순위 | 영역 | 핵심 내용 |
|---------|------|----------|
| **P0** | 멀티 인스턴스 라우팅 | 여러 Unity Editor 동시 제어, `editor current/select`, session/task별 editor pin, 응답 메타데이터에 target editor/project 명시 |
| **P0** | Workflow 번들 도구 | raw command 위에 `workflow compile-fix`, `workflow ui-smoke`, `workflow build-verify` 같은 멀티스텝 자동화 계층 추가 |
| **P0** | Visual Verification v2 | before/after screenshot diff, UI click/assert helper, play-mode smoke result bundle, console/hierarchy delta evidence 묶음 |
| **P1** | 제작 도메인 확장 | packages, camera/Cinemachine, URP/HDRP volume + renderer features, ScriptableObject, texture import/settings, shader/shader graph |
