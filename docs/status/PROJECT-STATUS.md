...
| `dotnet test tests/Unityctl.Core.Tests -c Release` | ✅ | 121 통과 |
| `dotnet test tests/Unityctl.Cli.Tests -c Debug` | ✅ | 387 통과 |
...
| Unityctl.Core.Tests | 121 |
| Unityctl.Cli.Tests | 387 |
| Unityctl.Mcp.Tests | 22 |
| Unityctl.Integration.Tests | 23 (환경 의존 3개 실패 가능) |

테스트 인벤토리 기준 합계는 **624개**다.
...
1. 멀티 인스턴스 라우팅 + agent pinning UX 정리
2. workflow 번들 계층과 evidence 묶음 설계
3. visual verification v2 범위 확정 (`screenshot diff`, UI assertion, play-mode smoke)
