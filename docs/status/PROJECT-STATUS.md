...
테스트 인벤토리 기준 합계는 **400개**다 (Script v1 + Diagnostics 테스트 포함).
...
| 스크립트 편집 | `create_script`, `script_apply_edits`, `validate_script` | `script-*` 4개 (create/edit/delete/validate) | ~85% (text edits 미지원) |
...
| 영역 | 구현 난이도 | ROI |
|------|-----------|-----|
| 읽기/탐색 API (hierarchy, find, component 값 조회) | 중 | 높음 — 수정 전 상태 파악 필수 |
| Graphics/렌더링 (33 액션) | 높음 | 중 — URP/HDRP 프로젝트에서 중요 |
| 스크린샷/시각 피드백 | 중 | 높음 — before/after 비교 |
| 멀티 인스턴스 라우팅 | 중 | 중 — 동시 에디터 제어 |
...
10. ~~**script editing v1 (create/edit/delete/validate)**~~ ✅ 완료 (2026-03-19)
11. **다음 개발 로드맵**: `docs/ref/phase-roadmap.md` "다음 개발 로드맵" 섹션 참조 (P0 읽기/탐색 API ~ P7 전문화 도메인)
